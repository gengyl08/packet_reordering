...
def measure(kernel, flow_num, cross_flow):

	print '>>>>>>>>>> Measuring with {} flows and cross flow {}'.format(flow_num, cross_flow)
	...
	taskset = ''
	if flow_num == 1:
		taskset = ' taskset -c 20'
	receiver_thread = subprocess.Popen('runlocalssh googlesh -uroot -corp -d1 -m{0} killall tcp_stream;{1} /home/ygeng/nettools/tcp_stream -H localhost -T {2} -F {3}'.format(receiver, taskset, min(flow_num, 8), flow_num).split(), stdout=subprocess.PIPE)
	time.sleep(5)

	# start sender
	print '>>>>>>>>>> Start sender'
	sender_thread = subprocess.Popen('runlocalssh googlesh -uroot -corp -d1 -m{} killall tcp_stream; /home/ygeng/nettools/tcp_stream -c -H {} -l 80 -T {} -F {}'.format(sender, receiver, min(flow_num, 8), flow_num).split())
	time.sleep(10)
	...
	# dump trace
	print '>>>>>>>>>> Start tcpdump'
	receiver_tcpdump_thread = subprocess.Popen('runlocalssh googlesh -uroot -corp -d1 -m{} /home/akabbani/nettools/tcpdump -s 128 -c 1000 -w /root/traces/multi_flow/{} host {}'.format(receiver, kernel + '_' + str(flow_num) + '_' + str(cross_flow), sender_ip).split())
	sender_tcpdump_thread = subprocess.Popen('runlocalssh googlesh -uroot -corp -d1 -m{} /home/akabbani/nettools/tcpdump -s 128 -c 1000 -w /root/traces/multi_flow/{} host {}'.format(sender, kernel + '_' + str(flow_num) + '_' + str(cross_flow), receiver_ip).split())
	receiver_tcpdump_thread.communicate()
	sender_tcpdump_thread.communicate()
	print '>>>>>>>>>> End tcpdump'

	sender_thread.communicate()
	throughput, err = receiver_thread.communicate()
	...
	th_result = None
	for line in throughput.split('\n'):
		if 'throughput_Mbps' in line:
			th_result = float(line.split('=')[-1])
			break;
...
def main():
	...
	f = open('results', 'a')
	f.write(version + '\n')
	f.write('flow_num\tthroughput_cross\tthroughput\tcpu\tInPkts_receiver\tInSegs_receiver\tbatch_ratio\tTCPOFOQueue_receiver\tOutSegs_receiver\tOutSegs_sender\tRetransSegs_sender\tTCPDSACKRecv_sender\n')
	for cross_flow in [0, 6000]:
		for flow_num in [1, 4, 16, 64, 128, 256]:
			th_cross_result, th_result, cpu_result, InPkts_receiver, InSegs_receiver, batch_ratio, TCPOFOQueue_receiver, OutSegs_receiver, OutSegs_sender, RetransSegs_sender, TCPDSACKRecv_sender = measure(version, flow_num, cross_flow)
			f.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(flow_num, th_cross_result, th_result, cpu_result, InPkts_receiver, InSegs_receiver, batch_ratio, TCPOFOQueue_receiver, OutSegs_receiver, OutSegs_sender, RetransSegs_sender, TCPDSACKRecv_sender))
